...
export default function PaymentIntent(props) {
  ...
  const {bookingToBuy, setIsAlreadyPayed} = props
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:5005/api/observatory/create-payment-intent", {
      ...
      body: JSON.stringify({ item: bookingToBuy }),
    })
      .then((res) => res.json())
    ...
    <div className="App">
      {clientSecret && (
        <Elements options={options} stripe={stripePromise}>
          <CheckoutForm isPayed={isPayed} setIsPayed={setIsPayed} />
        </Elements>
      )}
    </div>
  );
